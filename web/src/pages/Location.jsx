  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="flex flex-col md:flex-row justify-between items-center mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Location de Véhicules</h1>
          <p className="text-gray-600">Louez un véhicule ou proposez le vôtre</p>
        </div>
        <Link 
          to="/location/ajouter" 
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredVehicules.map((vehicule) => (
            <div key={vehicule.id} className="bg-white rounded-xl shadow-sm hover:shadow-md transition-shadow overflow-hidden border border-gray-100">
      ...
      {!loading && filteredVehicules.length === 0 && (
        <div className="text-center py-12">
          ...
          <p className="text-gray-500">Soyez le premier à proposer un véhicule !</p>
        </div>
      )}
    </div>
  )
